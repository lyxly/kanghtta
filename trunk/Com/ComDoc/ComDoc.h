...
how is?
  ...
  1：首先，判断header中MSAT开始扇区号是不是-2，MSAT大小是不是0，如果是-2，表示头中的msat的前109个sid并没有用完，只处理前109个扇区；
  2：如果，开始扇区号不是-2，表示msat占用了其它扇区，处理开始的109个扇区后，读header0x44处sid代表的扇区，判断该扇区最后一个
  sid是不是-2，如果不是，将除最后一个sid的其它sid保持在msat链中，读入该扇区的最后一个sid代表的扇区数据，重复2，直到
  读入的扇区id为free，末尾的sid为-2为止；

  一个扇区能存128个sid，这里，如果文件头0x44偏移处不为-2，那么读取改sid的扇区值，其扇区中存有128个sid，如果0x48>1，表示有多个扇区用来
  存储sat，那么判断第128个扇区处是否为-2，为-2表示结束，否则，每个扇区最后一个sid表示下个mast扇区的sid，依次处理；
...
/*
	what is sat;
	SAT是Sid的数组，它包含所有使用的流（短流除外，长度小于头中ULONG _ulMiniSectorCutoff的流为短流,除root外) 
	，内部控制流，SAT数组的大小和复合文档中存在的扇区数相等，可以用来求文件大小；
	通过读并连接MSAT中包含的扇区内容来构建SAT数组，扇区必须按照MAST中的sid的顺序读写
	SAT中的sid可以为-1，为-1的扇区在文件中存在，只是复合文档不使用。。每个流的链以-2结束，被SAT使用的扇区用-3标示，
	并不在任何流的链中，被MSAT使用的扇区id在SAT中被标示为4；
	每一个流的入口点总可以在某一地方获得。。可能是目录中，也可能是头文件中；
	
	SAT，读取MSAT中每个扇区，将其中的值链在一起，构成SAT数组，在对SAT进行处理。。
