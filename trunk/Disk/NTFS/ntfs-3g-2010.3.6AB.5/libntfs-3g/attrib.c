...
static int borrow_from_hole(ntfs_attr *na, runlist_element **prl,
    		s64 pos, s64 count, VCN *pupdate_from, BOOL wasnonresident,
		BOOL uncompressed_data)
{
	...
	if ((allocated < endblock) && (zrl->lcn == LCN_HOLE)) {
		...
		runlist_element *orl = na->rl;
			 ...
			 */
		if (!uncompressed_data && ntfs_attr_map_whole_runlist(na)) {
...
int curwrite = ++writenum;
	...
	uncompressed_data = compressed && NAttrNonResident(na)
		   && na->rl
		   && (na->initialized_size <= na->compressed_size)
		   && (na->initialized_size < na->compression_block_size);
	/*
...
recurse--;
	...
	/* Find the runlist element containing the vcn. */
// na->rl not always available so far
	rl = ntfs_attr_find_vcn(na, pos >> vol->cluster_size_bits);
	if (!rl) {
		/*
	...
	if (compressed) {
		if ((rl->lcn == (LCN)LCN_HOLE)
			...
			if (rl->lcn >= 0) {
				...
				compressed_part = borrow_from_hole(na,
					...
					wasnonresident,uncompressed_data);
			}
		}
		...
		if (compressed_part < 0)
			...
			/* just made non-resident, so not yet compressed */
...
errno_set:
	total = -1;
compressed = TRUE; // temporary to avoid warning
	goto out;
}
